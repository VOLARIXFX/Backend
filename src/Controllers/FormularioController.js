...
export const PostFormulario = async(req, res) => {
    try {
        ...
        const { tipo_operacion, divisa_operada, precio_apertura, precio_cierre, stop_loss, take_profit, hora_apertura, hora_cierre, resultado, notas } = req.body

        const fecha_post = new Date().toISOString().slice(0, 19).replace('T', ' ')
        ...
        let profit = 0;
        const apertura = parseFloat(precio_apertura);
        const cierre = parseFloat(precio_cierre);

        if (tipo_operacion === 'buy') {
            profit = cierre - apertura;
        } else if (tipo_operacion === 'sell') {
            profit = apertura - cierre;
        }

        await conexion.query(
            'insert into formulario (id_usuario, tipo_operacion, divisa_operada, precio_apertura, precio_cierre, stop_loss, take_profit, hora_apertura, hora_cierre, resultado, fecha_post, notas, imagen, profit) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
            [id_usuario, tipo_operacion, divisa_operada, precio_apertura, precio_cierre, stop_loss, take_profit, hora_apertura, hora_cierre, resultado, fecha_post, notas, imagen, profit]
        );
